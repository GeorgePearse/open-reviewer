...
export function prepareRunConfig(
  ...
  if (process.env.OPENROUTER_API_KEY) {
    console.log("Configuring for OpenRouter...");
    customEnv.ANTHROPIC_BASE_URL = "https://openrouter.ai/api/v1";
    customEnv.ANTHROPIC_API_KEY = process.env.OPENROUTER_API_KEY;
    // Ensure the model is set if provided via ANTHROPIC_MODEL env var (which comes from inputs.model)
    // The caller (index.ts) passes model via options, but here we are setting env vars.
    // claude-code CLI reads ANTHROPIC_MODEL env var if --model arg is not passed?
    // Actually, runClaude receives 'model' in options.
  }
